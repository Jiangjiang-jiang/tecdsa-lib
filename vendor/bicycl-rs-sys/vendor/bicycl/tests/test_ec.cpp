...
/* */
bool
test_ECNIZK (RandGen & randgen, SecLevel seclevel, size_t niter)
{
  ...
  {
    ECNIZKProof::SecretValue s (E.random_mod_order(randgen));
    ECNIZKProof::PublicValue Q (E, s);

    ECNIZKProof proof (E, H, randgen, s, Q);
...
bool check (RandGen & randgen, SecLevel seclevel, size_t niter)
{
  ...
  success &= test_ECNIZK (randgen, seclevel, niter);
