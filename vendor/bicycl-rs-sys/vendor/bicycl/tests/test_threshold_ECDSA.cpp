...
// TODO add tests with malicious users to validate the proofs
...
/* */
bool
test_commitments (RandGen & randgen, const thresholdECDSA &C, size_t niter, const string &pre)
{
  bool ret = true;
  ...
  {
    BN k (C.ec_group().random_mod_order(randgen));
    ECPoint Q (C.ec_group(), k);
    tie(c, r) = C.commit (Q, randgen);
    ret &= C.open (c, Q, r);
  }
...
/* */
bool test_ISetup (SecLevel seclevel,
                  size_t niter,
                  RandGen & randgen,
                  const string & pre)
{
  ...
  for (size_t iter = 0; iter < niter; iter++)
  {
    try
    {
      ...
      /* Check rho values match for all players*/
      Mpz rho = data2.at(0).rho();
      for (unsigned int i = 1; i < n; i++)
      {
        if (rho != data2.at(i).rho())
          throw LogicalError("rho values do not match");
      }

      /* CL generation */
      thresholdECDSA C0(seclevel, data2.at(0));
      for (unsigned int i = 0; i < n; i++)
      {
        thresholdECDSA Ci(seclevel, data2.at(i));
        if (not(C0.CL().h() == Ci.CL().h()) or
            not(C0.CL().q() == Ci.CL().q()) or
            not(C0.CL().M() == Ci.CL().M()) or
            not(C0.CL().DeltaK() == Ci.CL().DeltaK()))
          throw LogicalError("GenCL outputs do not match");
      }
    }
  ...
  string line (pre + " interactive setup");
  Test::result_line (line, ret);
  return ret;
}
           ...
           const string &pre)
{
  ...
  for (size_t iter = 0; iter < niter; iter++)
  {
    ...
    try
    {
      ...
      {
        signdata2.emplace(
            i, Sign2(C, randgen, signdata1.at(i), sk[i], Co_map, C1, ZK1));
      }
      ...
      /* check */
      BN k(0UL), gamma(0UL), delta;
      for (unsigned int i: S)
      {
        BN ki (signdata1.at(i).k_part());
      ...
      for (unsigned int i: S)
      {
        signdata5.emplace (i, Sign5 (C, randgen, signdata1.at(i), signdata2.at(i),
      ...
      for (unsigned int i: S)
      {
        signature.emplace(
            i,
            Signature(
                C, signdata1.at(i), signdata5.at(i), sk[i], s_map, hashed));
      }
      ...
      for (unsigned int i: S)
      {
        ...
        if (!C.verify (signature.at(i), sk[i].public_key(), hashed))
        {
...
bool check (SecLevel seclevel, RandGen &randgen, size_t niter)
{
  ...
  success &= test_commitments (randgen, C, niter, desc.str());
  success &= test_ISetup (seclevel, niter, randgen, desc.str());
