...
impl DecryptionKey {
    ...
    pub fn from_primes(p: Integer, q: Integer) -> Result<Self, Error> {
        ...
        let pm1 = (&p - 1u8).complete();
        let qm1 = (&q - 1u8).complete();
        let ek = EncryptionKey::from_n((&p * &q).complete());
        let lambda = pm1.lcm_ref(&qm1).complete();
    ...
    pub fn decrypt(&self, c: &Ciphertext) -> Result<Plaintext, Error> {
        ...
        if (&plaintext << 1u32).complete() >= *self.n() {
            Ok(plaintext - self.n())
        } else {
            Ok(plaintext)
        }
...
impl Drop for DecryptionKey {
    fn drop(&mut self) {
        self.lambda = Integer::from(0u8);
        self.mu = Integer::from(0u8);
        self.p = Integer::from(0u8);
        self.q = Integer::from(0u8);
    }
}
