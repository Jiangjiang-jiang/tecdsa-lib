...
/// Adapter that bridges `rand_core 0.6::RngCore` to the `glass_pumpkin 1.10`
/// expected interface (`rand_core 0.10::Rng` via `TryRng<Error = Infallible>`).
struct RngAdapter<'a, R: rand_core::RngCore + ?Sized>(&'a mut R);

// glass_pumpkin 1.10 requires `R: Rng + ?Sized` which auto-derives from
// `TryRng<Error = Infallible>`. We implement `rand_core 0.10::TryRng` directly.
impl<R: rand_core::RngCore + ?Sized> rand_core_v10::TryRng for RngAdapter<'_, R> {
...
impl Integer {
    ...
    pub fn is_probably_prime(&self, _reps: u32, rng: &mut impl rand_core::RngCore) -> IsPrime {
        if self.cmp0().is_le() {
            IsPrime::No
        } else if glass_pumpkin::prime::check_with(self.0.magnitude(), &mut RngAdapter(rng)) {
            IsPrime::Yes
        } else {
            IsPrime::No
        }
    ...
    pub fn generate_prime(rng: &mut impl rand_core::RngCore, bit_size: u32) -> Self {
        ...
        loop {
            ...
            x |= 1u32;
            if glass_pumpkin::prime::check_with(x.0.magnitude(), &mut RngAdapter(rng)) {
                break x;
            }
        }
    }
