...
Combines two measured sources, mirroring build_twoparty_table.py:
  ...
  * MtA (ms)   -- the TOTAL sender + receiver time of one MtA conversion,
                  ...
                  estimates.json}; the most recent run (new/) wins.
  ...
  * Comm.\\ (KB) -- the per-instance communication, measured SEPARATELY for the
                  ...
                  (every file merged, newest wins per key).
...
table's ZK-proof column:
  ...
  CL           : WMY23-style MtAwc check (g^alpha)          -- *no* ZK proof
...
# MtA (ms) sums every phase; the phases are exactly the measured sender +
# receiver work of one conversion.
ROWS = [
    ...
    ("cggmp20", "PL (CGGMP20)", r"$\mathcal{R}_\text{enc},\mathcal{R}_\text{aff-g}$",
     ["sender_encrypt", "receiver_compute", "sender_decrypt"]),
    # CL: security via the WMY23-style MtAwc check (g^alpha gen + verify), not a
    # ZK proof -> column is "---" (per request).
    ("cl", "CL", r"---",
...
def main():
    ...
    def comm_kb(slug):
        ...
        if s is None and r is None:
            return None
        return ((s or 0) + (r or 0)) / 1024.0

    label_w = max(len(label) for _, label, _, _ in ROWS)
    zk_w = max(len(zk) for _, _, zk, _ in ROWS)
    for slug, label, zk, phases in ROWS:
        ms = fmt(mta_ms(slug, phases))
        kb = fmt(comm_kb(slug))
